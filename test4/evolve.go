...
type Population struct {
	...
	fitness   float32                          // The total fitness
...
func (p *Population) Evolve(fitness Fitness) {
	...
	p.fitness = sumFitness(p.Values, fitness)
	p.pool = (p.pool + 1) % 2
...
// sumFitness returns the total fitness
func sumFitness(pop []Evolver, fitnessOf Fitness) (sum float32) {
	for _, v := range pop {
		sum += fitnessOf(v)
	}
	return
